...
public class CharacterCreationScreen extends VBox {
    ...
    public record HeroClassOption(String id, String labelText) {}

    public CharacterCreationScreen(CombatService combatService, BiConsumer<HeroClassOption, String> onConfirmSelection) {
        setAlignment(Pos.CENTER);
        setSpacing(20);
        setPadding(new Insets(30));
        ...
        List<String> classIds = List.of("WarriorClass", "ArcherClass", "WizardClass", "AssassinClass");
        List<HeroClassOption> classOptions = new ArrayList<>();

        for (String classId : classIds) {
            int hp = combatService.getIntProperty(classId, "hasHP");
            int atk = combatService.getIntProperty(classId, "hasBaseDamage");
            String weapon = combatService.getStartingWeaponForClass(classId);

            String className = classId.replace("Class", "");
            String labelText = String.format("%s (%d HP, %d ATK) - Starts with %s",
                    className, hp, atk, weapon != null ? weapon : "Bare Hands");

            classOptions.add(new HeroClassOption(classId, labelText));
        }
        ...
        confirmBtn.setOnAction(e -> {
            ...
            if (selected != null && onConfirmSelection != null) {
                ...
                String startingSkill = combatService.getStartingSkillForClass(selectedOption.id());
